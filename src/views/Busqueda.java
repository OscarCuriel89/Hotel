...
public class Busqueda extends JFrame {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if(filaReservas>=0) {
					...
					if(confirmaH==JOptionPane.YES_OPTION) {
						String valor = tbHuespedes.getValueAt(filaHuespedes, confirmaH).toString();
						huespedesControl.Elimiar(Integer.valueOf(valor));
	...
	private List<Reserva> buscarIdrserva(){
	...
	private void mostrarTablaReservasID() {
		List<Reserva> reserva = buscarIdrserva();
		modelo.setRowCount(0);
		try {
			for(Reserva reservas : reserva) {
				modelo.addRow(new Object[] {
	...
	private String calcularValorReserva(LocalDate dataE, LocalDate dataS) {
	...
	private void MostrarTablaHuespedes() {
		...
		private void actualizarHuespedes() {
			...
			.ifPresentOrElse(filaHuespedes ->{
				...
				Date fechaNacimiebnto= Date.valueOf(modeloHuesped.getValueAt(tbHuespedes.getSelectedRow(), 3).toString());
				...
				if(tbHuespedes.getSelectedColumn()== 0 || tbHuespedes.getSelectedColumnCount()==6) {
					JOptionPane.showMessageDialog(this, "no se puede modificar los id");
				}else {
					this.huespedesControl.ActualizarH(nombre, apellido, null, nacionalidad, telefono, id_reserva, id_reserva);
					JOptionPane.showMessageDialog(this, String.format("Registro modificado"));
				}
